...
public final class MarkableInputStream extends InputStream {
  ...
  public MarkableInputStream(InputStream in, ByteArrayPool pool) {
    this(in, DEFAULT_BUFFER_SIZE, pool);
  }

  public MarkableInputStream(InputStream in, int size, ByteArrayPool pool) {
    if (!in.markSupported()) {
      in = new RecyclableBufferedInputStream(in, size, pool);
    }
    this.in = in;
  }
